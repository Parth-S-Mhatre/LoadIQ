...
const Dashboard = () => {
  ...
  const NavIcon = ({ icon, to, tooltip }) => {
    ...
    return (
      <Link
        to={to}
        className={`group relative p-3 rounded-2xl transition-all duration-300 w-12 h-12 flex items-center justify-center ${active
    ...
    return (
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          <motion.div variants={itemVariants} className="col-span-2 navy-card rounded-2xl p-8">
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`px-6 py-2 rounded-xl text-xs font-black tracking-widest transition-all ${viewMode === 'load' ? 'bg-[#6366F1] text-white' : 'bg-slate-800 text-slate-400'}`}
                >
                  LOAD (MW)
                </button>
                <button
                  onClick={() => setViewMode('price')}
                  className={`px-6 py-2 rounded-xl text-xs font-black tracking-widest transition-all ${viewMode === 'price' ? 'bg-[#6366F1] text-white' : 'bg-slate-800 text-slate-400'}`}
                >
                  PRICE (€)
                </button>
              </div>
  ...
  const PredictorInputForm = () => {
    ...
    return (
      <motion.div
        ...
        className="bg-gradient-to-br from-slate-900/70 to-indigo-950/30 border border-indigo-500/10 rounded-3xl p-8 md:p-10 shadow-2xl max-w-4xl mx-auto"
      >
        <h2 className="text-2xl md:text-3xl font-black text-white mb-2 tracking-tight">Load Prediction Engine</h2>
        <p className="text-slate-400 mb-10">Adjust key inputs to simulate German grid load forecast</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            ...
            />
            <div className="text-right text-sm text-indigo-300">{formData.hour}:00</div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={handleChange}
              className="w-full bg-slate-800/60 border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-300">Solar Generation (MW)</label>
            <input
              type="number"
              name="DE_solar_generation_actual"
              value={formData.DE_solar_generation_actual}
              onChange={handleChange}
              className="w-full bg-slate-800/60 border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500"
              placeholder="e.g. 12450"
            />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-300">Wind Generation (MW)</label>
            <input
              type="number"
              name="DE_wind_generation_actual"
              value={formData.DE_wind_generation_actual}
              onChange={handleChange}
              className="w-full bg-slate-800/60 border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="e.g. 18900"
            />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-300">Current Load (MW)</label>
            <input
              type="number"
              name="DE_load_actual_entsoe_transparency"
              value={formData.DE_load_actual_entsoe_transparency}
              onChange={handleChange}
              className="w-full bg-slate-800/60 border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
        ...
        <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-end">
          <button className="px-8 py-4 bg-slate-700 hover:bg-slate-600 text-white font-semibold rounded-2xl transition-all shadow-lg">
            Reset
          </button>
          <button className="px-8 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-black rounded-2xl transition-all shadow-xl shadow-indigo-600/30 flex items-center gap-3">
            <Play size={18} /> Run Prediction
          </button>
        </div>

        {/* You can connect this form to your ML model via API call here */}
      </motion.div>
    );
  };
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <select className="w-full appearance-none bg-[#1A2235] border border-white/5 rounded-xl px-4 py-3 text-sm font-semibold text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all cursor-pointer">
                  <option>XGBoost Regressor v2.1</option>
                  <option>Deep Neural Net (LSTM)</option>
                  <option>Random Forest v1.4</option>
                </select>
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-b from-[#0A0F1D] to-[#0B1221] text-slate-100 font-sans">
      ...
      <div className="relative z-10">
        {/* Sidebar / Bottom nav */}
        <aside className="fixed bottom-0 left-0 right-0 h-20 md:h-screen md:w-24 md:top-0 md:left-0 bg-[#0B1121] border-t md:border-r border-white/5 md:border-t-0 z-50 flex md:flex-col items-center justify-around md:justify-start px-6 md:px-0 md:py-8 transition-all">
          ...
          <nav className="flex md:flex-col gap-3 md:gap-4 w-full md:w-auto mt-4">
        ...
        <main className="md:ml-24 p-6 md:p-8 lg:p-10 pb-32 md:pb-12 min-h-screen">
          {/* Header */}
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6 text-white">
            <div className="flex items-center gap-4">
            ...
            <div className="flex gap-4 items-center">
              <button
                ...
                className="px-5 py-2.5 bg-[#6366F1] hover:bg-[#5356E1] text-white rounded-xl text-[11px] font-bold tracking-widest transition-all shadow-lg flex items-center gap-2"
              >
                <Save size={14} />
                SAVE REPORT
              </button>
              <button
                ...
                className="px-5 py-2.5 bg-transparent text-white rounded-xl text-[11px] font-bold tracking-widest transition-all flex items-center gap-2 border border-white/10 hover:bg-white/5"
              >
                <Settings size={14} />
                SIGN OUT
              </button>
              ...
              <div className="flex gap-2 ml-4">
                <div className="relative">
                  <select className="appearance-none bg-[#10162A] border border-white/10 rounded-xl px-4 py-2 text-xs font-semibold text-slate-300 pr-8 cursor-pointer focus:outline-none focus:border-indigo-500">
                    <option>Week</option>
                    <option>Month</option>
                  </select>
